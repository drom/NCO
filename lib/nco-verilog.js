...
module.exports = config => {
    ...
    const reSelectSign = `${dataWidth*2+1}:${dataWidth+1}`;
    const imSelectSign = `${dataWidth}:0`;
    const reSign2Select = `${dataWidth*2+1}:${dataWidth}`;
    const imSign2Select = `${dataWidth}:1`;
...
assign ${pre}reSwap[${reSelect}] = ${pre}phase[0]^${pre}phase[1]?ncolut[${pre}addr][${imSelect}]:ncolut[${pre}addr][${reSelect}];
assign ${pre}imSwap[${imSelect}] = ${pre}phase[0]^${pre}phase[1]?ncolut[${pre}addr][${reSelect}]:ncolut[${pre}addr][${imSelect}];
...
assign ${p.i[0].wire}[${imeSelect}]=${p.t[0].wire}[${imSign2Select}];
...
`;
    ...
    n2({width: angleWidth + 2*dataWidth, capacity: 1})(rlsb)
